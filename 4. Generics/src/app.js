...
const arr1 = []; // This is equals to arr Which was defined above.
...
const mergeObject1 = merge({ name: "kjhlkjh" }, { age: 25 });
...
const mergeObject3 = merge1({ name: "xdfgddfg", hobbies: ["lsjnkfljknf"] }, { age: 30 });
...
// KeyOf Constraint:
function extactAndConvert(obj, key) {
    return "Your name is:" + " " + obj[key];
}
console.log(extactAndConvert({
    name: "Spurgeon",
}, "name"));
// Generic Classes
class DataStorage {
...
function createCourseGoal(title, description, completeUrl) {
    return {
        title,
        description,
        completeUrl,
...
function createCourseGoal1(title, description, completeUrl) {
    let goal = {};
    // In JS we can add properties to an objesct like this
    goal.title = title;
    goal.description = description;
